...
| RAG-01 | 系统维护检查与索引重建 | 在监控大屏中定位服务、索引或模型一致性问题 | P0 | 已实现 |
| CONFIG-01 | 运行配置维护 | 页面化调整 RAG 等非敏感运行参数 | P0 | 部分实现 |
...
| MONITOR-01 | 监控大屏 | 集中判断系统、模型与 RAG 链路健康度 | P1 | 部分实现 |
...
|---|---|
| 使用者 | 管理员、开发者 |
| 页面定位 | 监控大屏的“系统维护”二级视图，不作为独立导航页面；用于当前连通性检查和恢复操作 |
| 页面内容 | FastAPI、PostgreSQL、FAISS、Xinference、GPU 状态；当前 Chat / Embedding 模型；索引版本、维度和一致性；最近一次模型健康检查与告警 |
| 关键操作 | 刷新维护状态；索引不一致或缺失时执行重建；跳转至模型管理或日志查看关联问题 |
| 不包含 | 不提供模型选择、Endpoint / 密钥编辑或单模型健康检查按钮 |
| 结果反馈 | 显示服务可用性、索引一致性、当前模型及明确失败原因；重建成功后刷新索引状态 |
...
|---|---|
| 使用者 | 管理员 |
| 页面定位 | 非敏感应用行为配置页面；回答“系统应如何运行” |
| 页面内容 | 顶部横向展示当前 Chat / Embedding 模型摘要及跳转入口；中部采用三列布局展示“应用与连接”“RAG 行为”“当前生效摘要”；底部展示配置边界和运行说明 |
| 展示密度 | 默认显示常用字段；“展开全部参数”后补充数据库连接状态、FAISS 路径、Embedding Key 等低频只读字段，避免常规视图占用过多纵向空间 |
| 关键操作 | 刷新配置；展开/收起低频字段；跳转模型管理维护、切换 Chat / Embedding 模型；切换 Embedding 后跳转监控大屏的系统维护视图重建索引 |
| 不包含 | 不展示全量服务健康监控；不提供模型目录 CRUD、部署/停止、Endpoint / 密钥编辑或第二套模型选择控件 |
| 结果反馈 | 显示配置保存结果和生效范围；模型管理是 Chat / Embedding 的唯一选择与激活入口 |
...
flowchart TB
    H[运行配置
当前生效的非敏感应用参数] --> M[当前模型横向摘要
Chat · Embedding · 模型管理 · 系统维护]
    M --> G[三列配置区]
    subgraph G
        A[应用与连接
环境 · 日志级别 · 在线 API · 数据库]
        R[RAG 行为
Chunk size · Overlap · Top-K · Mock 状态]
        E[当前生效摘要
Embedding Provider · Endpoint · 路由]
    end
    G --> D[展开全部参数
低频只读字段：连接状态 · FAISS 路径 · Key 状态]
    D --> B[配置边界
模型管理：模型维护
监控大屏 > 系统维护：连通性和索引恢复]
```
...
|---|---|
| 使用者 | 管理员、开发者 |
| 页面内容 | 顶部“模型登记 / Xinference 运行时”双 Tab；模型登记视图采用筛选区、带分页的主表与右侧详情区；Xinference 运行时视图独立展示运行时列表；新增/编辑使用弹窗 |
| 主表字段 | 名称、UUID、Code、类型、来源、部署方式、启用状态、运行状态、健康状态、当前使用、更新时间、操作 |
| 筛选条件 | 名称或 Code、类型（Chat / Embedding / Rerank / Vision）、来源（官方 API / 中转站 / 本地）、部署方式（仅本地：Xinference / Ollama）、启用、运行、健康、当前使用状态 |
| 新增/编辑字段 | 自动生成且只读的 UUID、名称、唯一 Code、启用开关、备注、类型、来源；其余字段依来源和部署方式动态显示；Chat 模型额外提供允许的调用参数（如 temperature、top_p、max_tokens） |
| 表单联动 | 来源为官方 API / 中转站时，显示模型标识、Endpoint、密钥引用与 Chat 调用参数；来源为本地时才显示部署方式；本地 + Xinference / Ollama 时均可调用对应 API 获取模型列表，选择后自动回填模型信息；Xinference 额外展示已选模型摘要、Embedding 维度与本地启动参数；本地 Chat 模型可同时保存调用参数 |
| 关键操作 | 新增、编辑、删除、启用/停用、健康检查、设为当前；本地模型可部署/启动、停止。删除必须使用 Element Plus 确认弹窗二次确认，并明确不会停止运行时或删除本地缓存 |
...
```mermaid
flowchart TB
    A[筛选区\n名称/Code · 类型 · 来源 · 部署 · 状态] --> B[模型登记主表\n名称 · 类型 · 来源 · 部署方式 · 启用 · 运行 · 健康 · 当前]
    B --> C[右侧详情\nUUID（只读）· Endpoint · 密钥引用 · 运行参数]
    C --> D[编辑 / 健康检查 / 设为当前]
    B --> E[新增 / 编辑弹窗]
    E --> F{来源}
    F -->|官方 API / 中转站| G[模型标识\nEndpoint\n密钥引用]
    F -->|本地| H[部署方式\nXinference / Ollama]
    H -->|Xinference| I[获取可用模型\n选择后自动回填：名称、Code、类型、模型标识、维度与运行参数]
    H -->|Ollama| J[获取本地模型\n选择后自动回填：名称、Code、类型、模型标识与 Endpoint]
    B --> K[Xinference 运行时 Tab\n缓存 · 下载/加载 · 运行 · 停止]
```
...
|---|---|
| 使用者 | 管理员、开发者 |
| 页面目标 | 以聚合指标、趋势图、状态卡片和告警列表判断系统整体健康度，并承载“系统维护”二级视图；不替代日志的单请求排障能力 |
| 刷新方式 | 页面打开时加载；默认每 30 秒刷新，可手动刷新；异常状态需醒目标记 |
| 页面结构 | 顶部为页面标题、最近刷新时间、自动刷新开关、手动刷新和“总览 / 系统维护”二级切换；总览展示趋势、指标和告警；系统维护仅展示服务连通性、索引一致性和恢复操作，不重复展示趋势图或资源指标 |
...
当前实现范围：已接入 FastAPI / PostgreSQL、FAISS / Embedding 状态、模型目录、Chat 与入库日志、当前模型、Token 存量、近 7 日请求量、知识库文档与 Chunk 存量；RAG 配置与索引不一致、索引缺失、Embedding 未就绪、PostgreSQL 不可连接及监控数据源加载失败会生成页面告警。GPU、Xinference 运行状态、费用、在线用户、检索耗时、空检索率、模型 P95 / 失败率、指标快照持久化和阈值告警仍待后端采集与聚合接口接入，页面必须显示“—”或“尚未接入采集”，不得以模拟值替代。

#### 低保真原型：监控大屏

```mermaid
flowchart TB
    H[监控大屏<br/>最近刷新：10:30:00 · 自动刷新：30 秒 · 手动刷新]
    H --> T{二级视图}
    T -->|总览| S[整体健康度：正常 / 关注 / 异常<br/>异常数 · 当前 Chat 模型 · 当前 Embedding 模型]
    T -->|总览| K[核心指标卡片<br/>今日请求数 · 成功率 · Chat Token · 在线估算费用 · GPU 显存]
    T -->|系统维护| M0[服务连通性<br/>FastAPI · PostgreSQL · FAISS · Xinference]
    T -->|系统维护| M4[索引一致性与恢复<br/>模型版本 / 维度 · 重建索引]
    ...
    subgraph L[主监控区]
        ...
        subgraph R[请求与 RAG]
            direction TB
            R1[请求趋势<br/>请求量 · 成功率 · 失败数]
            R2[Token 趋势与模型占比<br/>输入 / 输出 / 总 Token]
            R3[RAG 效果<br/>开启率 · 命中率 · 空检索率 · 平均检索耗时]
        end
        subgraph M[模型与资源]
            direction TB
            M1[模型状态<br/>Chat / Embedding · 健康检查 · 连续失败]
            M2[模型性能<br/>平均耗时 · P95 · 失败率 · Embedding 请求/文本/向量]
            M3[基础设施状态<br/>FastAPI · PostgreSQL · FAISS · Xinference · GPU 利用率 / 显存]
        end
    end

    L --> D[知识库概览<br/>文档数 · Chunk 数 · 今日入库数 · 入库成功率 · 索引版本 / 最近重建]
    D --> A[告警列表<br/>级别 · 对象 · 触发时间 · 状态 · 说明 · 查看关联日志]
    A --> G[跳转日志页面定位关联记录]
    M0 --> MM[前往模型管理]
    M4 --> RI[确认后重建索引]
```

原型布局规则：状态与核心指标固定在首屏；趋势图只呈现聚合数据；告警列表固定放在页面底部并按未恢复、告警级别、最新触发时间排序。点击告警的“查看日志”应保留对象、时间范围和事件类型筛选条件跳转到日志页面。
...
1. 管理员在模型管理页选择已登记的 Chat 或 Embedding 模型并“设为当前”。
2. 系统保存激活状态，并将该登记记录中的模型标识、Endpoint、密钥引用与允许的运行参数应用于后续请求；Chat 调用参数仅允许 `temperature`、`top_p`、`max_tokens`、`presence_penalty`、`frequency_penalty`；API 重启后从模型目录恢复。
3. 若切换 Chat 模型，后续对话立即生效；激活模型后不会静默回退到 `.env` 中另一套 Chat 模型。
4. 若切换 Embedding 模型、维度或 Provider，模型管理页标记索引需要重建，并跳转至监控大屏的“系统维护”视图执行恢复操作。
5. 管理员在监控大屏的“系统维护”视图确认重建后，系统从 PostgreSQL 中的文档重新向量化并更新 FAISS。

### 5.3.1 页面职责与跳转

1. 用户想确认服务、索引或资源是否正常时，进入“监控大屏”；“总览”用于聚合趋势和告警，“系统维护”用于当前连通性检查与恢复。
2. 用户想修改 RAG 或应用运行行为时，进入“运行配置”；该页面只修改非敏感运行参数，不管理模型实例。
3. 用户想登记、编辑、启停、检查或切换某个模型时，进入“模型管理”；Chat / Embedding 的选择和激活只有该页面提供。
4. 系统维护检测到模型不可用时，提供“前往模型管理”入口；模型管理切换 Embedding 后，提供“前往监控大屏 > 系统维护重建索引”入口；告警详情提供“前往日志”入口。

### 5.4 模型登记、可用性与运行管理

1. 管理员通过“新增模型”弹窗创建模型，系统自动生成 UUID；管理员填写名称、唯一 Code、类型、来源、启用状态和备注。编辑操作使用同一弹窗，并回填已有配置。
...
4. 来源为本地且部署方式为 Xinference 时，管理员点击“获取可用模型”调用 Xinference API，再从返回列表中选择模型；模型标识不允许手工填写。
5. 选择 Xinference 模型后，系统自动回填模型标识、模型类型、Embedding 维度、模型引擎、模型格式、量化方式和下载源到 `runtime_config`；表单显示已选模型摘要。高级区域允许管理员调整 GPU、下载源等运行参数。
6. 来源为本地且部署方式为 Ollama 时，管理员点击“获取可用模型”调用 Ollama API；选择后自动回填模型名称、Code、类型、模型标识和 Endpoint，不显示 Xinference 专属字段。
...
9. 删除仅删除平台内的模型登记记录，不删除 Xinference 模型缓存或本地模型文件；删除当前使用模型前必须先切换同类型模型。删除前必须展示 Element Plus 确认弹窗，说明影响范围并由管理员二次确认。
...
3. 监控大屏按固定时间窗口聚合日志、状态检查和资源采集数据。总览展示当前值与趋势；系统维护视图仅展示服务连通性、索引一致性与恢复操作，不直接展示原始问题或密钥。
4. 当前已接入的状态异常（RAG 配置/索引不一致、索引缺失、Embedding 未就绪、PostgreSQL 不可连接和数据源加载失败）必须生成页面告警，展示级别、对象、触发时间、当前状态、说明和日志跳转入口；阈值型告警在 `metric_snapshots` 与 `alerts` 后端聚合能力接入后生成或更新。
...
- “系统维护”作为监控大屏二级视图，仅展示服务连通性、索引一致性和恢复操作；不作为独立导航页面，不提供模型选择、Endpoint / 密钥编辑或单模型检查操作。
- 运行配置页面仅维护非敏感运行参数；不重复提供模型选择、模型部署/停止或全量系统监控。
- 模型管理是 Chat / Embedding 选择、激活与单模型健康检查的唯一入口；跨页跳转保留问题对象和必要上下文。
...
- 监控大屏以“总览 / 系统维护”二级视图呈现：总览展示 RAG、模型、基础设施和知识库指标的当前值及趋势；系统维护仅处理连通性和索引恢复，并可跳转模型管理或关联日志。
