...
| CONFIG-01 | 当前 Chat / Embedding 模型切换 | 在模型管理中统一控制推理与检索模型 | P0 | 已实现 |
...
|---|---|
| 使用者 | 管理员 |
| 页面内容 | 应用、数据库、Embedding、RAG 等非敏感运行状态；提供模型管理入口 |
| 关键操作 | 跳转模型管理页维护和切换模型；本页不提供第二套模型选择或 Endpoint / 密钥配置 |
| 结果反馈 | 说明模型管理是 Chat / Embedding 的唯一配置入口；切换 Embedding 时明确提示需要重建索引 |

#### 低保真原型：模型切换

```mermaid
flowchart TB
    subgraph C[当前 Chat 模型]
        C0[当前：DeepSeek Chat\n中转站 · GeekAI]
        C1[可选：Qwen 2.5 7B\n本地 · Xinference · 运行中]
        C2[可选：GPT-5 mini\n官方 API]
        C3[应用 Chat 切换]
    ...
    subgraph E[当前 Embedding 模型]
        E0[当前：BGE Base ZH v1.5\n768 维 · 本地 Xinference]
        E1[可选：BGE M3\n1024 维 · 本地 Xinference]
        E2[可选：text-embedding-3-small\n1536 维 · 官方 API]
        E3[应用 Embedding 切换]
        E4[提示：索引需重建\n跳转至系统状态页]
        E0 --> E3
        E1 --> E3
...
| 页面结构 | 顶部为页面标题、最近刷新时间、自动刷新开关和手动刷新；首屏先展示整体健康度与核心指标；中部按“请求与 RAG”“模型与资源”两列呈现趋势和状态；底部展示知识库概览和需要处理的告警列表 |
...
flowchart TB
    ...
    H --> S[整体健康度：正常 / 关注 / 异常<br/>异常数 · 当前 Chat 模型 · 当前 Embedding 模型]
    H --> K[核心指标卡片<br/>今日请求数 · 成功率 · Chat Token · 在线估算费用 · GPU 显存]
