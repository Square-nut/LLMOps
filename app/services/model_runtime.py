...
EMBEDDING_PROVIDERS = {"xinference", "tei", "geekai", "openai", "mock"}
...
def validate_catalog_model(model: Dict[str, Any]) -> None:
    ...
    if model_type == "embedding" and provider not in EMBEDDING_PROVIDERS:
        raise ValueError("Embedding runtime must be xinference, tei, geekai, openai, or mock")
...
def validate_activatable_model(model: Dict[str, Any]) -> None:
    ...
    if model_type == "embedding":
        if provider in {"xinference", "tei"} and not model.get("endpoint"):
            raise ValueError("Xinference / TEI embedding requires a service endpoint")
...
def apply_model(model: Dict[str, Any]) -> None:
    ...
    if model_type == "chat":
        ...
        if provider == "geekai":
            ...
            settings.geekai_api_key = _DEFAULT_GEEKAI_API_KEY
        elif provider == "openai":
            settings.geekai_base_url = model["endpoint"].rstrip("/") if model.get("endpoint") else _OPENAI_API_BASE
            settings.geekai_api_key = _DEFAULT_OPENAI_API_KEY
        else:
    ...
    if model_type == "embedding":
        ...
        if provider in {"xinference", "tei"}:
            settings.embedding_provider = "local"
            settings.embedding_backend = "tei"
            settings.embedding_api_base = model["endpoint"].rstrip("/")
            settings.embedding_api_key = _DEFAULT_EMBEDDING_API_KEY or provider
            ...
            settings.embedding_api_key = _DEFAULT_GEEKAI_API_KEY
        elif provider == "openai":
            settings.embedding_provider = "openai"
            settings.embedding_api_base = (
                model["endpoint"].rstrip("/") if model.get("endpoint") else _OPENAI_API_BASE
            )
            settings.embedding_api_key = _DEFAULT_OPENAI_API_KEY
        else:
